...
4 失敗パターン (UX 判定軸) と Credo 4 原則 (`.claude/rules/credo.md`) は以下のように対応する。
UX レビュー時は第一階層として 4 失敗パターンで判定し、第二階層として対応する Credo 原則の問いを当てる。

| 4 失敗パターン | 対応する Credo 原則 | 接続の根拠 |
|---|---|---|
| 伝わらない | **Spec vs Evidence** — シナリオ上は伝わるはずなのに実際は伝わらないなら、シナリオの方を更新する | ユーザーの実体験がスペックより優先 |
| ぼやける | **Core First, Polish Later** — core flow より装飾が目立っているなら、優先順位が逆転している | 装飾が core を邪魔していないか |
| 見つからない | **Wire Before You Decorate** — 部品が配線されていても画面上で発見できないなら vertical slice が未完成 | 機能の存在と発見可能性は別 |
| 始まらない | **Core First, Polish Later** — 初見ユーザーが core flow に入れないなら、core への導線が不足している | core flow への最短到達を問う |

**No Dead Code** は 4 失敗パターンと 1:1 対応しないが、「使われない UI 要素が画面を
ぼやけさせる」「到達不能な画面が存在する」場合に横断的に適用する。
...
- **運用統合 (γ2)**: 完了 (Issue #28)。`CLAUDE.md` / `git-workflow.md` / `ux-review.md` / `uxaudit.md` / `README.md` に uxaudit 運用手順を追記済み
