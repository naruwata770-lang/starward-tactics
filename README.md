...
Phase 1 〜 10 の機能スキャフォールディング (盤面 / ユニット / URL 共有 / PNG 出力 /
レイアウト仕上げ) は **全フェーズ完了済み**。本番公開 (Vercel) + CI/Hook 整備まで
ひと通り揃っている。

以降のフェーズは Phase 番号ではなく **機能ドリブンの Issue 運用** に切り替え済み。
次に着手するのは以下のトピック群:

- **UX / アクセシビリティ**: ARIA combobox 化 / キーボードナビ / 情報密度調整
- **quiz 画像作成向け機能**: 被ロック数表示 / 撃破状態強化 / キャラアイコン表示
- **コミュニティ機能 (将来)**: 保存・共有・コメント等 (DB 統合タイミングで別途計画)

現在の backlog とステータスは [GitHub Issues](../../issues) に集約してある。
